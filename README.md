...
### [Added] - 2026-09-01
- **Laporan Evaluasi Data Riil Lapangan (Web Report)**: Menambahkan route `/laporan-usulan-riil` di `routes/web.php` dan generator laporan HTML Light Mode (`python/build_laporan_usulan_riil.py`) untuk transparansi publik pengujian 50 data riil usulan crowdsourcing dan log pencarian lapangan.
- **Evaluasi & Pengujian Statistik KBLI 2025**: Script komputasi M1 s.d. M6 (`python/run_full_m1_m6_eval_50.py`), uji hipotesis berpasangan (Bootstrap CI 95%, Permutation Test, Wilcoxon Signed-Rank Test), serta audit validasi 100% kode KBLI 2025 ground truth.
...
- **Penyempurnaan Form Edit Kode & Kolom Tabel Pengajuan**:
  ...
  - **Optimasi Kolom Kode & Tabel Pengajuan**: Menampilkan judul klasifikasi langsung di bawah kode dengan lebar kolom tetap (fixed width `220px`) dan teks otomatis turun ke bawah (*wrap text*), serta menghilangkan kolom `created_at` agar tabel lebih lapang dan memudahkan proses verifikasi/approval.
...
- **Tabel Pengajuan Contoh Lapangan**: Mengubah tab filter menjadi berdasarkan status (Semua, Belum Disetujui, Sudah Disetujui) dilengkapi dengan *badge* jumlah data, penambahan filter Jenis KBLI/KBJI, serta menambahkan pengurutan default dari yang terbaru (`created_at` descending).
